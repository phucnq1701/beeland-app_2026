...
import { Search, Bell, MapPin, ChevronRight, Sparkles, Calendar, Users, Heart, ClipboardList, Phone, Clock } from 'lucide-react-native';
...
export default function HomeScreen() {
  ...
  const loadFeatureConfiguration = async () => {
    try {
      ...
      console.log('[Home] Load feature config error:', error instanceof Error ? error.message : String(error));
      const defaultFeatures = features.slice(0, MAX_HOME_FEATURES);
      setDisplayedFeatures(defaultFeatures);
      initializeFeatureAnimations(defaultFeatures.length);
    }
  ...
  const initializeFeatureAnimations = (count: number) => {
    featuresAnimations.current = Array(count).fill(0).map(() => new Animated.Value(0));
  ...
  const handlePressProperty = useCallback((id: string) => {
    animatePress(`property-${id}`, () => {
      try {
        console.log('[Home] Navigate to project options', { id });
      ...
      >
        ...
        >
          {featuredProperties.map((property, index) => (
            <Animated.View
              key={property.id}
              style={{
                transform: [{ scale: getCardScale(`property-${property.id}`) }]
              }}
            >
              <TouchableOpacity
                ...
                style={[
                  ...
                  index === featuredProperties.length - 1 && styles.propertyCardLast,
                ]}
                activeOpacity={1}
                onPress={() => handlePressProperty(property.id)}
              >
                <Image
                ...
                <View style={styles.propertyBadge}>
                  ...
                  <Text style={styles.propertyBadgeText}>Đang bán</Text>
                </View>
        ...
        <View style={styles.featuresSection}>
          ...
          <View style={styles.featuresGrid}>
            {displayedFeatures.map((feature, index) => {
              const animValue = featuresAnimations.current[index] || new Animated.Value(1);
              return (
                <Animated.View
                  key={feature.id}
                  style={{
          ...
          {bookings.slice(0, 5).map((booking) => (
            ...
            >
              <View style={[
                ...
                { backgroundColor: booking.status === 'paid' ? 'rgba(16,185,129,0.1)' : booking.status === 'waiting' ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)' }
              ]}>
                <ClipboardList size={20} color={booking.status === 'paid' ? Colors.success : booking.status === 'waiting' ? Colors.warning : Colors.error} />
              </View>
              <View style={styles.recentCardContent}>
                <Text style={styles.recentCardTitle} numberOfLines={1}>{booking.customerName}</Text>
                <Text style={styles.recentCardSub} numberOfLines={1}>{booking.productCode} • {booking.projectName}</Text>
                <View style={styles.recentCardRow}>
                  <Calendar size={12} color={Colors.textTertiary} />
                  <Text style={styles.recentCardSub} numberOfLines={1}>{booking.bookingDate} → {booking.expiryDate}</Text>
                </View>
              </View>
              <View style={styles.recentCardRight}>
                <Text style={styles.recentCardAmount}>{booking.amount}đ</Text>
                <View style={[
                  styles.statusBadge,
                  { backgroundColor: booking.status === 'paid' ? 'rgba(16,185,129,0.1)' : booking.status === 'waiting' ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)' }
                ]}>
                  <Text style={[
                    ...
                    { color: booking.status === 'paid' ? Colors.success : booking.status === 'waiting' ? Colors.warning : Colors.error }
                  ]}>
                    {booking.status === 'paid' ? 'Đã TT' : booking.status === 'waiting' ? 'Chờ TT' : 'Hết hạn'}
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
        ...
        <View style={styles.recentSection}>
          ...
          {customers.slice(0, 5).map((customer) => (
            ...
            >
              <View style={[styles.recentIconBox, { backgroundColor: 'rgba(59,130,246,0.1)' }]}>
                <Users size={20} color={Colors.accent.blue} />
              </View>
              <View style={styles.recentCardContent}>
                <Text style={styles.recentCardTitle} numberOfLines={1}>{customer.name}</Text>
                <View style={styles.recentCardRow}>
                  <Phone size={12} color={Colors.textTertiary} />
                  <Text style={styles.recentCardSub} numberOfLines={1}>{customer.phone}</Text>
                </View>
                <View style={styles.recentCardRow}>
                  <Clock size={12} color={Colors.textTertiary} />
                  <Text style={styles.recentCardSub} numberOfLines={1}>Tạo: {customer.createdAt}</Text>
                </View>
              </View>
              <View style={[
                styles.statusBadge,
                { backgroundColor: customer.status === 'active' ? 'rgba(16,185,129,0.1)' : customer.status === 'potential' ? 'rgba(59,130,246,0.1)' : 'rgba(200,200,200,0.15)' }
              ]}>
                <Text style={[
                  styles.statusBadgeText,
                  { color: customer.status === 'active' ? Colors.success : customer.status === 'potential' ? Colors.accent.blue : Colors.textTertiary }
                ]}>
                  {customer.status === 'active' ? 'Hoạt động' : customer.status === 'potential' ? 'Tiềm năng' : 'Không HĐ'}
                </Text>
              </View>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.recentSection}>
          ...
          {appointments.slice(0, 5).map((apt) => (
            <TouchableOpacity
              ...
              onPress={() => router.push('/appointments')}
            >
              <View style={[styles.recentIconBox, { backgroundColor: 'rgba(16,185,129,0.1)' }]}>
                <Calendar size={20} color={Colors.success} />
              </View>
              <View style={styles.recentCardContent}>
                <Text style={styles.recentCardTitle} numberOfLines={1}>{apt.title}</Text>
                <View style={styles.recentCardRow}>
                  <Clock size={12} color={Colors.textTertiary} />
                  <Text style={styles.recentCardSub} numberOfLines={1}>{apt.date} • {apt.startTime}-{apt.endTime}</Text>
                </View>
              </View>
              <View style={[
                styles.statusBadge,
                { backgroundColor: apt.status === 'confirmed' ? 'rgba(16,185,129,0.1)' : apt.status === 'pending' ? 'rgba(245,158,11,0.1)' : apt.status === 'completed' ? 'rgba(59,130,246,0.1)' : 'rgba(239,68,68,0.1)' }
              ]}>
                <Text style={[
                  styles.statusBadgeText,
                  { color: apt.status === 'confirmed' ? Colors.success : apt.status === 'pending' ? Colors.warning : apt.status === 'completed' ? Colors.accent.blue : Colors.error }
                ]}>
                  {apt.status === 'confirmed' ? 'Xác nhận' : apt.status === 'pending' ? 'Chờ XN' : apt.status === 'completed' ? 'Hoàn thành' : 'Đã hủy'}
                </Text>
              </View>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.recentSection}>
          ...
          {products.slice(0, 5).map((product) => (
            ...
            >
              <View style={[styles.recentIconBox, { backgroundColor: 'rgba(236,72,153,0.1)' }]}>
                <Heart size={20} color={Colors.accent.pink} />
              </View>
              <View style={styles.recentCardContent}>
                <Text style={styles.recentCardTitle} numberOfLines={1}>{product.code}</Text>
                <Text style={styles.recentCardSub} numberOfLines={1}>{product.price}đ</Text>
              </View>
              <View style={[
                styles.statusBadge,
                { backgroundColor: product.status === 'available' ? 'rgba(16,185,129,0.1)' : product.status === 'pending' ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)' }
              ]}>
                <Text style={[
                  styles.statusBadgeText,
                  { color: product.status === 'available' ? Colors.success : product.status === 'pending' ? Colors.warning : Colors.error }
                ]}>
                  {product.status === 'available' ? 'Còn hàng' : product.status === 'pending' ? 'Đang giữ' : 'Đã hủy'}
                </Text>
              </View>
            </TouchableOpacity>
          ))}
